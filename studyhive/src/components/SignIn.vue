        ...
        <div class="w-1/2 h-full bg-white flex flex-col justify-center items-center gap-y-5">
            ...
            <div class="singup w-full h-full bg-white flex flex-col justify-center items-center gap-y-5">
              ...
              <div class="flex flex-row w-[70%] gap-x-7">
                <button class="btn rounded-3xl w-48" popovertarget="popover-1" style="anchor-name:--anchor-1">
                  Month
                </button>
                <ul class="dropdown menu w-52 rounded-box bg-base-100 shadow-sm"
                  popover id="popover-1" style="position-anchor:--anchor-1">
                  <li v-for="month in months"><a>{{month}}</a></li>
                </ul>
                <button class="btn rounded-3xl w-48" popovertarget="popover-2" style="anchor-name:--anchor-2">
                  Day
                </button>
                <ul class="dropdown menu w-52 rounded-box bg-base-100 shadow-sm max-h-[30rem]"
                  popover id="popover-2" style="position-anchor:--anchor-2">
                  <li v-for="day in days"><a>{{day}}</a></li>
                </ul>
                <button class="btn rounded-3xl w-48" popovertarget="popover-3" style="anchor-name:--anchor-3">
                  Year
                </button>
                <ul class="dropdown menu w-52 rounded-box bg-base-100 shadow-sm max-h-[30rem]"
                  popover id="popover-3" style="position-anchor:--anchor-3">
                  <li v-for="year in years"><a>{{year}}</a></li>
                </ul>
              </div>
              ...
              <button class="cursor-pointer w-[15rem] h-[4rem] rounded-2xl bg-gradient-to-r from-amber-500 to-pink-400">Sign Up</button>
              <button class="cursor-pointer w-[15rem] h-[4rem] rounded-2xl bg-white border-2">alr have an acc? sign in</button>
            </div>
          <div class="signin w-full h-full bg-white flex-col justify-center items-center gap-y-5 hidden">
